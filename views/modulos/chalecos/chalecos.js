class Chalecos{
    ...
    filtrarTabla(desde,hasta){
        if( (new Date(desde).getTime() >= new Date(hasta).getTime()))
        {
            ...
            $.ajax({
                ...
                url: "views/modulos/chalecos/selectChalecos.php",
            })
             .done(function( data, textStatus, jqXHR ) {
                ...
                data.forEach(datos => {
                    ...
                    this.contentTablaChac+="</span> creo el Chaleco : <span  class='text-primary' ' onclick='modal.cargarChaleco("+datos.idObjeto+")'>"+datos.serie+" </span></td>";
                    this.contentTablaChac+="</tr>";
    
                    console.log( this.contentTablaChac);
                });
        console.log( "Chalecos cargados!");
    ...
    $('#tablaChalecos').hide(50);
    $('#cuerpoChalecos').html( this.contentTablaChac);
                     }else{
                        $('#tablaChalecos').show(50);
                        $('#cuerpoChalecos').html( this.contentTablaChac);
                        var table = $('#tablaChalecos').DataTable({
                            language: {
                                "decimal": "",
