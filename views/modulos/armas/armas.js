class Armas{
    ...
    filtrarTabla(desde,hasta){
        if( (new Date(desde).getTime() >= new Date(hasta).getTime()))
        {
            ...
            $.ajax({
                ...
                url: "views/modulos/cascos/selectArmas.php",
            })
    ...
    console.log(datos)
                    ...
                    this.contentTablaFunc+="</span> creo el arma  <span  class='text-primary' data-toggle='modal' data-target='modalFunc' onclick='modal.cargarArma("+datos.idObjeto+")'>"+datos.rut+" </span></td>";
                    this.contentTablaFunc+="</tr>";
    
                    console.log( this.contentTablaFunc);
                });
             
                     console.log( "Armas cargados!");
    console.log(this.contentTablaFunc);
    console.log( "Armas cargados!");
    ...
    $('#tablaArmas').hide(50);
    $('#cuerpoArmas').html( this.contentTablaFunc);
                     }else{
                        $('#tablaArmas').show(50);
                        $('#cuerpoArmas').html( this.contentTablaFunc);
                        var table = $('#tablaArmas').DataTable({
                            language: {
                                "decimal": "",
